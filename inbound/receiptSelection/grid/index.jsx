...
export default function ReceiptSelection() {
    ...
    const PAGE_KEY = "inbound_receiptSelection";
    const Detail_PAGE_KEY = "receiptSelection_Details";
    ...
    const items = [{ label: 'Receipt' }, { label: 'Receiving Confirmation' }];
    ...
    const loadLazyDataInExecution = () => {
        ...
        networkTimeout.current = setTimeout(() => {
            ReceiptsDashboardService.getInExecutionReceipts(lazyStateInExecution)
                .then((data) => {
    ...
    const loadLazyDataAllReceipts = () => {
        ...
        networkTimeout.current = setTimeout(() => {
            ReceiptsDashboardService.getAllReceipts(lazyStateAllReceipts)
                .then((data) => {
    ...
    const canViewDetails = hasPageAccess(Detail_PAGE_KEY);
    ...
    return (
        <div>
            <div>
                {canViewDetails ? (
                    <Link
                        to={`/inbound/receiptSelection/${rowData.id}`}
                        style={{
                            fontSize: '18px',
                            fontWeight: 8000,
                            textDecoration: 'none'
        ...
        <>
            ...
            <div className="card">
                <TabView activeIndex={activeIndex} onTabChange={(e) => setActiveIndex(e.index)}>
                    {/* First tab */}
                    <TabPanel header="In Execution">
                        <h3>Receipts in Execution</h3>
