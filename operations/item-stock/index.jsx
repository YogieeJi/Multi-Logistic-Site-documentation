    ...
    const onSubmitReason = () => {
        ...
        ItemsStockService.addReservationReason( (data) ).then((data) => {
            ...
            if(data.error == 0){
                ...
                toast.current.show({
                    severity: 'error',
                    sticky: true, // stays until manually closed
                    content: (props) => {
                        const errors = Array.isArray(data.errors)
                            ? data.errors.map((err, i) => <div key={i}>{err}.</div>)
                            : <div>{data.errors}</div>;
    ...
    return (
        ...
        <div className="card">
            ...
            >
               ...
               <Column field="prd_PrimaryCode" style={{ width: '10rem' }}  header="Item" body={rowdata=>(rowdata.prd_PrimaryCode)} filterMenuStyle={{ width: '25rem' }}  showFilterMenu={false} sortable filter filterPlaceholder="Search" />
               <Column field="prdl_Description" header="Item Description" body={descriptionBodyTemplate} filterMenuStyle={{ width: '20rem' }}  style={{ width: '20rem' }}  showFilterMenu={false} sortable filter filterPlaceholder="Search" />
                ...
                <Column field="ProductPackTypeDescription" header="Pack type" body={packBodyTemplate} filterMenuStyle={{ width: '14rem' }} showFilterMenu={false} sortable filter filterPlaceholder="Search" />
