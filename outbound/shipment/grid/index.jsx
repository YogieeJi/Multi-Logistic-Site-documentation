...
export default function shipment() {
    const { hasActionAccess } = useAuth();
        const PAGE_KEY = "outbound_shipment";
    ...
    const optionsList = (selectedDelivery || []).map(delivery => ({
        label: `${delivery.shp_Code} - ${delivery.messageName}`,
        value: delivery.shp_ID
    }));
    ...
    const multiselectFooterTemplate = () => {
        const length = selectedLanes ? selectedLanes.length : 0;
    ...
    const items = [{ label: 'Outbound' }, { label: 'Shipment' }];
    ...
    const onSelectionChange = (event) => {
        const value = event.value; // updated selection for current page

        setSelectedDelivery(prevSelected => {
            const currentPageIds = delivery.map(d => d.shp_ID);

            // Keep selections from other pages
            const remainingSelections = prevSelected.filter(
                item => !currentPageIds.includes(item.shp_ID)
            );

            // Merge and remove duplicates by shp_ID
            const merged = [...remainingSelections, ...value].filter(
                (item, index, self) =>
                    index === self.findIndex(t => t.shp_ID === item.shp_ID)
            );
        ...
        // Check if all current page records are selected
        setSelectAll(value.length === delivery.length);
    };
    ...
    const onSelectAllChange = (event) => {
        ...
        if (selectAllChecked) {
            ...
            setSelectedDelivery(prevSelected =>
                prevSelected.filter(item => !delivery.some(d => d.shp_ID === item.shp_ID))
            );
        }
    ...
    const actionItems = [
        ...
        hasActionAccess(PAGE_KEY, "release_locations") &&{
            label: 'Release Locations',
            icon: 'pi pi-minus',
            command: () => {
                if (selectedDelivery != null && selectedDelivery.length > 0) {
        ...
        hasActionAccess(PAGE_KEY, "mark_shipment_complete") &&{
            ...
            command: () => {
                if (selectedDelivery != null && selectedDelivery.length > 0) {
                    ...
                    setbtnDisabled(true)
                    onclick(setMarkShipmentDisplayConfirmation(true))
                } else {
                    toast.current.show({ severity: 'error', summary: 'Error', detail: 'Atleast select 1 order' });
                }
            }
        },
        hasActionAccess(PAGE_KEY, "assign_truck_dock_multiple") &&{
            label: 'Assign Truck & Dock Location for Multiple Shipments',
            icon: 'pi pi-plus',
            command: () => {
                if (selectedDelivery != null && selectedDelivery.length > 0) {
    ...
    const header = (
        <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
            ...
            <span className="block mt-2 md:mt-0 p-input-icon-left">
                {hasActionAccess(PAGE_KEY, "create_shipment") &&(<Button label="Create Shipment" icon="pi pi-sync" severity="sucess" onClick={() => navigate("/outbound/shipment-add")} />)}
            </span>
        </div>
    );
    ...
    const removeDelivery = (idToRemove) => {
        setSelectedDelivery(prevSelected =>
            prevSelected.filter(item => item.shp_ID !== idToRemove)
        );
    };
    ...
    const onDropdownOpen = () => {
        document.body.classList.add('hiidenn');
    };

    const onDropdownClose = () => {
        document.body.classList.remove('hiidenn');
    };
        ...
        <>
            ...
            <div className="card">
                ...
                >
                    ...
                    <Column field="shp_Code" header="Code " body={codeBodyTemplate} filterMenuStyle={{ width: '14rem' }} showFilterMenu={false} sortable filter filterPlaceholder="Search" />
                    {/* <Column field="shp_ID"  header="Ship ID" body={(rowData) => rowData.shp_ID} filterMenuStyle={{ width: '14rem' }} showFilterMenu={false} /> */}
                    <Column field="minCustomerOrderCode" header="Customer Code" body={(rowData) => rowData.minCustomerOrderCode} filterMenuStyle={{ width: '14rem' }} showFilterMenu={false} />
                    <Column field="truckCodePlate" sortable header="Truck" body={trk_IDBodyTemplate} showFilterMenu={false} filter filterPlaceholder="Search" />
                    <Column field="dispatchMethodCodeName" header="Dispatch Method" body={dispatch_methodBodyTemplate} filterMenuStyle={{ width: '14rem' }} showFilterMenu={false} sortable filter filterPlaceholder="Search" />
                    <Column field="loc_Code" sortable header="Location" body={location_idBodyTemplate} showFilterMenu={false} filter filterPlaceholder="Search" />
                    ...
                    <Column field="messageName" sortable header="Status" body={statusBodyTemplate} showFilterMenu={false} filter filterPlaceholder="Search" />
                    {hasActionAccess(PAGE_KEY, "delete")&&(<Column field="dlt" header="Action" body={dltBodyTemplate} showFilterMenu={false} />)}
                </DataTable>
            </div>
        </>
