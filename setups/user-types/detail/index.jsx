...
export default function DeliveryDetails() {
    ...
    const [lazyState, setlazyState] = useState({
        ...
        filters: {
            ...
            // ost_Weight: { value: '', matchMode: 'contains' },
            // weight: { value: '', matchMode: 'contains' },
            // ost_Volume: { value: '', matchMode: 'contains' },
            // ost_VolumeUnitID: { value: '', matchMode: 'contains' },
    ...
    const onSelectAllChange = (event) => {
        ...
        if (selectAll) {
            DeliveryService.getDeliveryLines(params.id).then((data) => {
                setSelectAll(true);
                setSelectedDeliveryLines(data.shipments);
            });
        } else {
            setSelectAll(false);
    ...
    return (
        <>
            ...
            <div className="grid">
                ...
                >
                    ...
                    <Column field="ost_Code"  header="Code " body={ost_OrderIDBodyTemplate} filterMenuStyle={{ width: '14rem' }}  sortable  filter filterPlaceholder="Search" showFilterMenu={false}/>
                                ...
                                {/* <Column field="ost_Weight" header="Weight" body={ost_WeightBodyTemplate} filterMenuStyle={{ width: '14rem' }} showFilterMenu={false} sortable  filter filterPlaceholder="Search" />
                                <Column field="weight"  header="Weight Unit " body={ost_WeightUnitIDBodyTemplate} filterMenuStyle={{ width: '14rem' }} showFilterMenu={false} sortable   filter filterPlaceholder="Search" />
                                <Column field="ost_Volume" sortable header="Volume" body={ost_VolumeDBodyTemplate} filter filterPlaceholder="Search" showFilterMenu={false}  />
                                <Column field="ost_VolumeUnitID" header="Volume Unit" body={ost_VolumeUnitIDBodyTemplate} filterMenuStyle={{ width: '14rem' }}showFilterMenu={false}  sortable  filter filterPlaceholder="Search" /> */}
                                ...
                                {/* <Column field="Agency" header="Agency" body={AgencyBodyTemplate} filterMenuStyle={{ width: '14rem' }} showFilterMenu={false}  sortable  filter filterPlaceholder="Search"  /> */}
            ...
            <div class="flex justify-content-end flex-wrap">
                    <Button label="Cancel" onClick={() => navigate("/outbound/shipment")} type='button' severity="secondary" className="w-2 p-2 mt-2 mr-2 " outlined ></Button>
                    <Button label="Edit"   onClick={() => navigate("/outbound/shipment/edit/"+selectedRow?.shp_ID,{ state: { shp_ID: selectedRow?.shp_ID  } })} className="w-2 p-2 mt-2" ></Button>
                    </div>
               </div>   
               </div>  
               </div>
